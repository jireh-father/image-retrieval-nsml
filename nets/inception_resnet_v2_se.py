    ...
    with tf.variable_scope(scope, 'InceptionResnetV2', [inputs],
        ...
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            with tf.variable_scope('Logits'):
                ...
                # net = slim.fully_connected(net, 832, activation_fn=tf.nn.relu)

                # 2 layers
                # net = slim.dropout(net, dropout_keep_prob, is_training=is_training)
                # net = slim.fully_connected(net, 1067, activation_fn=tf.nn.relu)
                #
                # net = slim.dropout(net, dropout_keep_prob, is_training=is_training)
                # net = slim.fully_connected(net, 598, activation_fn=tf.nn.relu)

                # 3 layer
                # net = slim.dropout(net, dropout_keep_prob, is_training=is_training)
                # net = slim.fully_connected(net, 1184, activation_fn=tf.nn.relu)
                #
                # net = slim.dropout(net, dropout_keep_prob, is_training=is_training)
                # net = slim.fully_connected(net, 832, activation_fn=tf.nn.relu)
                #
                ...
                if isinstance(num_classes, list):
                    ...
                    #                    scope='Dropout2_1')
                    # # end_points['PreLogitsFlatten2'] = net
                    # net = slim.fully_connected(net, 1460, activation_fn=tf.nn.relu,
                    #                            scope='Logits2_1')
                    #
                    # net = slim.dropout(net, dropout_keep_prob, is_training=is_training,
                    #                    scope='Dropout2_2')
                    ...
                    # logits = end_points['AuxLogits']+logits
