...
fl.DEFINE_integer('log_every_n_steps', 5, 'The frequency with which logs are print.')
...
fl.DEFINE_string('model_name', 'inception_v4', '')
fl.DEFINE_boolean('use_crossentropy', True, '')
...
fl.DEFINE_boolean('use_one_pair_sampling', False, '')
...
fl.DEFINE_integer('train_image_size', 299, '')  # pnasnet_large 331, inception_resnet 299, resnet 224, inception_v4 299
...
# Triplet #
#######################
fl.DEFINE_integer('embedding_size', 2048, '')
fl.DEFINE_string('triplet_strategy', 'batch_hard', '')
fl.DEFINE_boolean('use_all_loss', False, '')
fl.DEFINE_boolean('use_focal_loss', False, '')
fl.DEFINE_float('focal_loss_sigma', 0.3, '')
...
# fl.DEFINE_float('weight_decay', 0.00001, '')
# fl.DEFINE_float('weight_decay', 0.0002, '')
# fl.DEFINE_float('weight_decay', 0.0001, '')
...
fl.DEFINE_float('opt_epsilon', 1e-3, 'Epsilon term for the optimizer.')
...
# fl.DEFINE_float('learning_rate', 0.0005733, 'Initial learning rate.')

# fl.DEFINE_float('learning_rate', 0.000002, 'Initial learning rate.')
# fl.DEFINE_float('learning_rate', 0.00005, 'Initial learning rate.')
# fl.DEFINE_float('learning_rate', 0.00253149, 'Initial learning rate.')
# fl.DEFINE_float('end_learning_rate', 0.0001, 'The minimal end learning rate used by a polynomial decay.')
fl.DEFINE_float('end_learning_rate', 0.00000001, 'The minimal end learning rate used by a polynomial decay.')
...
# Fine-Tuning Flags #
#####################
# fl.DEFINE_string('nsml_checkpoint', '70', '')
# fl.DEFINE_string('nsml_session', 'jireh_family/ir_ph2/176', '')
...
pretrained_map = {
    ...
    "inception_resnet_v2_se": "http://download.tensorflow.org/models/inception_resnet_v2_2016_08_30.tar.gz",
    "inception_resnet_v2_mh": "http://download.tensorflow.org/models/inception_resnet_v2_2016_08_30.tar.gz",
...
pretrained_filename_map = {
    ...
    "inception_resnet_v2_se": "inception_resnet_v2_2016_08_30.ckpt",
    "inception_resnet_v2_mh": "inception_resnet_v2_2016_08_30.ckpt",
...
checkpoint_exclude_scopes_map = {
    ...
    "inception_resnet_v2_se": "InceptionResnetV2/Logits,InceptionResnetV2/AuxLogits,InceptionResnetV2/se_MaxPool_5a_3x3,InceptionResnetV2/se_Mixed_5b,InceptionResnetV2/se_repeat1,InceptionResnetV2/se_Mixed_6a,InceptionResnetV2/se_repeat2,InceptionResnetV2/se_Mixed_7a,InceptionResnetV2/se_repeat3,InceptionResnetV2/se_Conv2d_7b_1x1",
    "inception_resnet_v2_mh": "InceptionResnetV2/Logits,InceptionResnetV2/AuxLogits",
...
def bind_model(saver, sess, images_ph, embeddings_op, cf):
    ...
    def infer(queries, db):
        ...
        dataset_queries = dataset_queries.map(_parse_function, num_parallel_calls=cf.num_preprocessing_threads)
        ...
        dataset_db = dataset_db.map(_parse_function, num_parallel_calls=cf.num_preprocessing_threads)
...
if __name__ == '__main__':
    ...
    if cf.mode == 'train':
        ...
        def train_pre_process(filename, label):
            image_string = tf.read_file(filename)
            image = tf.image.decode_jpeg(image_string, channels=cf.train_image_channel)

            if image_preprocessing_fn is not None:
                image = image_preprocessing_fn(image, cf.train_image_size, cf.train_image_size)
            else:
                image = tf.cast(image, tf.float32)
                # central_fraction = random.uniform(0.75, 0.999)
                central_fraction = 0.9
                # central_fraction = tf.random_uniform((), minval=0.75, maxval=0.999)
    ...
    if cf.mode == 'train':
        ...
        if cf.use_pair_sampling:
            if cf.use_one_pair_sampling:
                ai_ph = tf.placeholder(tf.int32, [None, 1], name="anchor_indices")
                pi_ph = tf.placeholder(tf.int32, [None, 1], name="positive_indices")
                loss_op, end_points, train_op, embeddings_op = model_fn.build_model(images_ph, labels_ph, cf, True,
                                                                                    num_examples, global_step,
                                                                                    class_cnt, anchor_indices=ai_ph,
                                                                                    positive_indices=pi_ph)
            else:
                loss_op, end_points, train_op, embeddings_op = model_fn.build_model(images_ph, labels_ph, cf, True,
                                                                                    num_examples, global_step,
                                                                                    class_cnt)
        ...
        embeddings_op = model_fn.build_model(images_ph, labels_ph, cf, is_training=False, class_cnt=class_cnt)
                ...
                try:
                    ...
                    if cf.use_pair_sampling:
                        ...
                        for i, tmp_label in enumerate(tmp_labels):
                            if tmp_label not in label_buffer and tmp_label not in one_label_buffer:
                                label_buffer[tmp_label] = i
                                one_label_buffer[tmp_label] = i
                            ...
                            if cf.use_one_pair_sampling and tmp_label in one_label_buffer:
                                anchor_indices.append([i])
                                positive_indices.append([one_label_buffer[tmp_label]])
                                del one_label_buffer[tmp_label]
                        ...
                        if cf.use_one_pair_sampling and len(anchor_indices) > cf.batch_size // 2:
                            anchor_indices = anchor_indices[:cf.batch_size // 2]
                            positive_indices = positive_indices[:cf.batch_size // 2]
                            # print("anchor", len(anchor_indices))
                    ...
                    if cf.use_pair_sampling:
                        ...
                        if cf.use_one_pair_sampling:
                            feed_dict[ai_ph] = anchor_indices
                            feed_dict[pi_ph] = positive_indices
